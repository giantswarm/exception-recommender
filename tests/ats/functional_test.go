...
var _ = Describe("exception-recommender", func() {
	...
	Describe("a failing policy with a warming PolicyManifest", Ordered, func() {
		policyName := "policy-warming"
		reportName := "report-warming"
		uid := types.UID("uid-warming")
	...
	DescribeTable("reports the reconciler should ignore",
		...
		Entry("excluded namespace", excludedNamespace, targetKind, targetCategory, "policy-excluded-namespace"),
		Entry("non-target workload kind", resourceNamespace, untargetedKind, targetCategory, "policy-untargeted-kind"),
		Entry("non-target category", resourceNamespace, targetKind, untargetedCategory, "policy-untargeted-category"),
		Entry("policy without a registered PolicyManifest", resourceNamespace, targetKind, targetCategory, "policy-unregistered"),
	)
})
